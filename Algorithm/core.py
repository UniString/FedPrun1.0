...
class Masking(object):
    def __init__(self, rand_indices_dic,optimizer, death_rate=0.3, growth_death_ratio=1.0, death_rate_decay=None, death_mode='magnitude', growth_mode='momentum', redistribution_mode='momentum', threshold=0.001, args=False, train_loader=False):
        ...
        self.modules = []
        self.names = []
        self.keep_layer=[]  #保存不稀疏的参数以备再添加到掩码矩阵
    ...
    def init(self, mode='ERK', density=0.05, erk_power_scale=1.0):
        ...
        if mode == 'global_magnitude':
            ...
            total_sparse_params = total_params * self.args.density_local
            ...
            for name, mask in self.masks.items():
                ...
                self.masks[name][:] = (torch.rand(mask.shape) < density_dict[name]).float().data.cuda(self.args.gpu)
        ...
        for module in self.modules:
            for name, weight in module.named_parameters():
                if name in self.masks:
                    ...
                    self.densitys[name]=(weight!=0).sum().item()/weight.numel()
                    total_size += weight.numel()
                    sparse_size += (weight != 0).sum().int().item()
                else:
                    self.densitys[name] = 1
    ...
    def add_module(self, module, density, sparse_init='ER'):
        ...
        self.init(mode=sparse_init, density=density)   #根据剪枝模式下一步操作
